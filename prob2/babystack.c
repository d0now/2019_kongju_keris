    ...
    while (true) {
        ...
        switch (ins.cmd) {
            ...
            case 2: // register type
                if (ins.arg >= 4) break;
                ret = stack_push(regs[ins.arg]);
                break;
            case 3: // value type
                ret = stack_push(ins.arg);
                break;

            /* pop instruction */
            case 4: // register type
                if (ins.arg >= 4) break;
                ret = stack_pop(&regs[ins.arg]);
                break;
            case 5: // value type
                ret = stack_pop(&reg_tmp);
                break;

            /* sp controll instruction */
            case 6:
                ret = stack_expand(ins.arg);
                break;
            case 7:
                ret = stack_shrink(ins.arg);
                break;
        ...
        fprintf(stderr, "\n");
        DPRINT("sp = %p", sp);
        DPRINT("regs[0] = 0x%08x", regs[0]);
        DPRINT("regs[1] = 0x%08x", regs[1]);
        DPRINT("regs[2] = 0x%08x", regs[2]);
        DPRINT("regs[3] = 0x%08x", regs[3]);
#endif
    }
